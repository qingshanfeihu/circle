...
界面：输入框是一圈彩虹框，忙碌时状态（动词、本轮 token、耗时、相位）嵌在上沿；页脚只有一行计量（token、模型、费用、缓存命中）。
对话里块与块之间恰好空一行：一段思考和紧跟的回答算一块，连续的工具调用算一块。工具行连同 `⎿` 结果按类型铺底色——
查阅（读文件、搜索、网页、skill）蓝、改文件绿、子代理青，思考行洋红；回答正文不铺底色。
